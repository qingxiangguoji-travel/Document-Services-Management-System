  ...
  <PageLayout>
          ...
          <el-col :span="6">
            <el-form-item label="代理人">
              <el-select v-model="filters.agent" placeholder="请选择代理公司 - 联系人" filterable clearable class="w-100">
                <el-option v-for="a in agentOptions" :key="a.unique_key" :label="a.display_label" :value="a.display_label" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="6">
    ...
    <el-card shadow="never" class="section-card table-card print-area">
      ...
      >
        ...
        <!-- ✅ 业务统计（父：摘要；子表头：字段名；子行：横向明细） -->
        <el-table-column label="业务统计" min-width="1060">
              ...
              <div class="order-summary-line">
                <span class="expand-btn" @click.stop="toggleOrder(scope.row.__orderId)">
                  <el-icon class="expand-icon">
                    <ArrowDown v-if="isOrderExpanded(scope.row.__orderId) || isCustomerModeForceExpand(scope.row.__orderId)" />
                    <ArrowRight v-else />
                  </el-icon>
                </span>

<span class="order-code-prefix">
  订单编号：<span class="code-font">{{ scope.row.code || scope.row.order_no || '-' }}</span>，
</span>
...
<strong class="agent-strong">
  {{ [scope.row.agent_company, scope.row.agent_contact].filter(Boolean).join(' - ') }}
              ...
              <div v-if="isCustomerView && scope.row.__customerHit" class="customer-hit-hint">
                ...
                <span v-if="scope.row.__customerHit.passport">({{ scope.row.__customerHit.passport }})</span>
              </div>
            </div>
            ...
            <div v-else class="biz-row-grid">
              ...
              <div class="biz-cell">{{ scope.row.alias || scope.row.staff_no || '-' }}</div>
              ...
              <div class="biz-cell biz-wide">
                <div v-if="scope.row.__feeLines?.length" class="fee-lines">
                  <div v-for="(it, idx) in scope.row.__feeLines" :key="idx" class="fee-line">
                    ...
                    <span class="fee-v">${{ Number(it.amount || 0).toLocaleString() }}</span>
                  </div>
                </div>
        ...
        <!-- ✅ 办理进度：父(统计块) / 子(状态) / 子表头(空) -->
        <el-table-column label="办理进度" width="160" align="center">
          <template #default="scope">
            <div v-if="scope.row.__rowType === 'order'" class="status-detail-tag" :class="`status-type-${scope.row.__statusInfo.type}`">
              <div v-for="(line, idx) in scope.row.__statusInfo.lines" :key="idx" class="status-line">
                <span class="dot"></span> {{ line }}
              </div>
            </div>

            <div v-else-if="scope.row.__rowType === 'subhead'" class="muted small-center"> </div>
        ...
        <el-table-column label="金额 (USD)" width="110" align="right">
          <template #default="scope">
            <span v-if="scope.row.__rowType !== 'subhead'" class="amount-bold">
              ${{ Number(scope.row.__finance?.amount || 0).toLocaleString() }}
            </span>
          </template>
        </el-table-column>

        <el-table-column label="上游费用" width="100" align="right">
          <template #default="scope">
            <span v-if="scope.row.__rowType !== 'subhead'">
        ...
        <el-table-column label="代理佣金" width="110" align="right">
          <template #default="scope">
            <template v-if="scope.row.__rowType === 'biz'">
              <el-input-number
            ...
            <template v-else-if="scope.row.__rowType !== 'subhead'">
        ...
        <el-table-column label="利润" width="110" align="right">
          <template #default="scope">
            <span
              ...
              :style="{ color: Number(scope.row.__finance?.profit || 0) >= 0 ? '#10b981' : '#ef4444', fontWeight: 700 }"
            >
        ...
        <el-table-column label="结算" width="110" align="center">
          <template #default="scope">
            <template v-if="scope.row.__rowType === 'order'">
              <el-dropdown trigger="click" @command="(cmd) => handleUpdateSettlement(scope.row.__orderId, cmd)">
                <el-tag
                  :type="scope.row.__settlement === 'paid' ? 'success' : (scope.row.__settlement === 'partial' ? 'warning' : 'danger')"
                  effect="plain"
                  class="cursor-pointer"
                >
                  {{ settlementText(scope.row.__settlement) }} <el-icon><ArrowDown /></el-icon>
                </el-tag>
                <template #dropdown>
                  <el-dropdown-menu>
        ...
        <el-table-column label="备注" width="140">
          <template #default="scope">
            <template v-if="scope.row.__rowType !== 'subhead'">
              {{ scope.row.remark || '-' }}
            </template>
          </template>
        </el-table-column>
              ...
              <el-dropdown trigger="click" @command="(cmd) => handleCommand(cmd, scope.row.__orderId)">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    ...
                    <el-dropdown-item command="delete" icon="Delete" divided style="color: #f56c6c">删除订单</el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
            </template>
...
const normalizeRowNo = (row, idx) => {
  // 新建页常见：row_no / _displayNo / display_no / no / seq
  const v =
    row.row_no ??
    row._displayNo ??
    row.display_no ??
  ...
  pagedOrders.value.forEach(order => {
    const bizRows = (order.customers || []).map((c, idx) => {
      ...
      return {
        ...
        __orderId: order.id,
        __rowIndex: idx,
        __rowNoDisplay: rowNo,
        __visaExpireAt: visaExpireAt || '',
        __workPermitText: c.has_work_permit ? '有' : (c.has_work_permit === false ? '无' : '-'),
    ...
    const orderRow = {
      ...
      __rowType: 'order',
      __orderId: order.id,
      __customersCount: bizRows.length,
    ...
    if (expanded) {
      // ✅ 子表头：每个订单展开时插一次
      rows.push({ __rowType: 'subhead', __orderId: order.id })
...
const handleSingleExport = (orderId) => {
  const order = filteredOrders.value.find(o => o.id === orderId) || allOrders.value.find(o => o.id === orderId)
  if (!order) return ElMessage.warning('未找到订单')

  const header = ['序号', '客户姓名', '护照号', '国籍', '签证到期日', '劳工证', '业务', '费用明细', '办理进度', '金额', '上游费用', '代理佣金', '客服提成', '利润', '结算']
  const rows = (order.customers || []).map((c, idx) => {
    const f = calcRowFinance(c)
    const feeLines = buildFeeLines(c).map(x => `${x.label}:${x.amount}`).join(' | ')
  ...
  downloadCSV(`订单_${normalizeOrderCode(order)}_明细`, header, rows)
}
