...
const isConfirmedOrderUI = computed(() => {
  // 只有 confirmed，或者 history 且确实有“外部来源”才算确认
  const hasRealSource =
...
const repairHistoryOrders = () => {
  ...
  const fixed = raw.map(o => {
    const isHistoryNotify =
      ...
      !o.source_notify_id &&   // 没来源
      !o.linked_order_id      // 没新单
  ...
  if (changed) {
    ...
    console.warn('[ORDER REPAIR] 历史通知订单数据已修复')
  }
}
