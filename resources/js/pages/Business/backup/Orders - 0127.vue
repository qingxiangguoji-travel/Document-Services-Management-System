      ...
      >
        ...
        <el-table-column label="办理进度" width="160" align="center">
          <template #default="scope">
            ...
            >
<div
  v-for="(line, idx) in scope.row.__statusInfo.lines"
  :key="idx"
  class="status-line"
  :class="`status-type-${line.ui}`"
>
  <span class="dot"></span> {{ line.text }}
</div>
...
const getDisplayStatusInfo = (bizRows) => {
  ...
  bizRows.forEach(r => {
    const statusCode = r.__processStatus
    const statusLabel = normalizeStatusLabel(statusCode)

    const settlement =
      r.__settlement === 'paid'
        ? '已付'
        : r.__settlement === 'partial'
        ? '部分结算'
        : '未付'
...
// 终态不拼结算前缀
const key =
  group === 'cancelled' || group === 'returned'
    ? statusLabel
    ...
    const g = getStatusGroup(statusCode)
  ...
  const lines = Array.from(counter.entries()).map(([text, v]) => ({
    text: `${v.count}人 ${text}`,
    ui: STATUS_GROUP_TO_UI[v.group] || 'info'
  }))
...
const loadData = () => {
  ...
  // ✅ 永远按 created_at 倒序（再用 confirmed_at 兜底）
  const sorted = [...raw]
    .filter(o => !o.deleted)
    .sort((a, b) => {
      const da = String(b.created_at || b.confirmed_at || b.converted_at || '')
      const dbb = String(a.created_at || a.confirmed_at || a.converted_at || '')
      return da.localeCompare(dbb) // YYYY-MM-DD / ISO 字符串可直接比较
    })
...
const filteredOrders = computed(() => {
  ...
  (filters.__processStatus === 'unfinished' &&
    ...
    return matchCode && matchAgent && matchDate && matchDept && matchType && matchNationality && matchCustomer && matchStatus
  })
})
...
watch(
  ...
  // 日期范围筛选（来自仪表盘）
  if (q.dateRange) {
    if (typeof q.dateRange === 'string' && q.dateRange.includes(',')) {
      const [start, end] = q.dateRange.split(',')
  ...
  // 办理状态筛选（来自仪表盘）
  if (q.processStatus) {
    // 我们用 customerName 触发展开模式不合适
    // 所以直接挂一个“隐藏过滤器”到 filters
    filters.__processStatus = q.processStatus
  }
